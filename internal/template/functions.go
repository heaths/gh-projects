...
func ago(t time.Time) string {
	...
	case d < time.Hour:
		approx = text.Pluralize(int(d.Minutes()), "minute")

	case d < 24*time.Hour:
		approx = text.Pluralize(int(d.Hours()), "hour")

	case d < 30*24*time.Hour:
		approx = text.Pluralize(int(d.Hours())/24, "day")

	case d < 365*24*time.Hour:
		approx = text.Pluralize(int(d.Hours())/24/30, "month")

	default:
		approx = text.Pluralize(int(d.Hours())/24/365, "year")
	}
